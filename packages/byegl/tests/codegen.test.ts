  ...
  test('shader generation', ({ gl }) => {
    ...
    expect(toWgsl(gl, glsl).wgsl).toMatchInlineSnapshot(`
      ...
      struct _Uniforms {
        ...
        @align(16) u_lights: array<Light, 3>,
      }
...
describe('logical expressions', () => {
  test('&& and || in if condition', ({ gl }) => {
    ...
    expect(toWgsl(gl, glsl).wgsl).toMatchInlineSnapshot(`
      ...
      fn _byegl_fake_fragment_0() {
        if (_uniforms.uA && _uniforms.uB || _uniforms.uC) {
          gl_FragColor = vec4f(1.0);
        }
      }
  ...
  test('|| nested inside && gets parenthesized', ({ gl }) => {
    const glsl = `
      void main() {
        bool a = true;
        bool b = false;
        bool c = true;
        // (a || b) && c — the || must be wrapped in parens
        bool z = (a || b) && c;
      }
    `;
  ...
  test('&& in variable initializer', ({ gl }) => {
    const glsl = `
      void main() {
        bool x = true;
        bool y = false;
