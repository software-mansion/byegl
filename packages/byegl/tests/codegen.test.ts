...
describe('arrays', () => {
  test('shader generation', ({ gl }) => {
    ...
    expect(toWgsl(gl, glsl).wgsl).toMatchInlineSnapshot(`
      ...
      struct _byegl_VertexOut_4 {
        @builtin(position) _byegl_posOut_5: vec4f,

      }


      @vertex
      fn _byegl_vert_main_6() -> _byegl_VertexOut_4 {


        _byegl_fake_vertex_0();
        var output: _byegl_VertexOut_4;
        output._byegl_posOut_5 = gl_Position;
        // NOTE: OpenGL uses z in the range [-1, 1], while WebGPU uses z in the range [0, 1].
        output._byegl_posOut_5.z = output._byegl_posOut_5.z * 0.5 + 0.5;

        return output;
      }

      @fragment
      fn _byegl_frag_main_7() -> @location(0) vec4f {
