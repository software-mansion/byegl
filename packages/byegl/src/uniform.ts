...
export type UniformValue = number | boolean | Float32Array | Uint32Array | Int32Array | boolean[];
...
export class UniformBufferCache {
  ...
  #serializeValue(
    ...
    if (dataType.type === 'bool') {
      ...
      // Booleans are actually not host-shareable, so we use
      // u32 to pass `0` or `1`.
      serialized = new Uint32Array([...(value as boolean[])].map((v) => (v ? 1 : 0))).buffer;
   ...
   */
  updateUniform(glLocation: ByeGLUniformLocation, value: UniformValue | Iterable<number>) {
