...
## Non-polyfill approach

You can also create a custom context directly:

```ts
// instead of: const gl = canvas.getContext('webgl2');
const gl = await byegl.createContext({ canvas, version: 2 /* WebGL 2 */ });
```

This allows you to use ByeGL on platforms that don't support WebGL natively (like React Native).
